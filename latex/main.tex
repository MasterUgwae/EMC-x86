...
Segmentation is a method of memory management that seperates parts of a processes address space into different segments (such as code and data), it uses the 6 segment registers to keep track of the segments.
...
The value in the segment register is left shifted by 4 (multiplied by 16) then added to the offset to produce a 20-bit physical address allowing up to 1MiB of memory instead of the 64KiB if segmentation was not used.
...
Add flat model, and other models, add gdt/ldt and their registers and add flags in the segment registers
