...
** Will never respond if another cache is Owned or Forward, otherwise depending on implementation either a single Shared cache will repond, they all respond or none respond (most likely).
...
\begin{enumerate}
  ...
  \item Associative: \\
    ...
    Every cacheline is checked in parrallel for if the tags are equal.
    On a hit, the word is used as an offset into the cacheline to retrieve the data.
...
Most x86 processors use N-Way Set-Accociative caches, with the more performant ones using larger set sizes.
