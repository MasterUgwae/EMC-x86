...
\begin{table}[H]
  ...
  \begin{tabular}{|c|c|l|}
    ...
    m & Memory address & A memory address such as [eax + 4*ebx + 4] \\
...
The types of an operand or identical instructions with different names will be seperated by a \texttt{|}, the size in bits may also be appended to the type. Furthermore, some instructions have more than one row of operands, each row shows a different combination of operand types

% TODO: jmps add, (arithmetic) xor, (logical) cmp test
...
Example code of using syscalls can be found on page \pageref{linuxsyscalls}.
...
This program prints "Hello World" to the terminal then exits without an error.
